              ...
              <tbody>
                {metrics.model_summary.map((item, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50">
                    ...
                    <td className="text-right py-2">{item.tokens}</td>
                    <td className="text-right py-2">${item.cost?.toFixed(4) || '0.0000'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {metrics.users_summary.map((user, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50">
                    ...
                    <td className="text-right py-2">{user.requests || 0}</td>
                    <td className="text-right py-2">{user.tokens || 0}</td>
                    <td className="text-right py-2">${user.cost?.toFixed(4) || '0.0000'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {metrics.recent_requests.map((req, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50">
                    ...
                    <td className="text-right py-2">{req.tokens}</td>
                    <td className="text-right py-2">${req.cost?.toFixed(4) || '0.0000'}</td>
                    <td className="py-2">{req.timestamp ? new Date(req.timestamp).toLocaleString() : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
